...
void ht_insert(ht_table_t *table, char *key, float value) {
  ...
  if (tmp == NULL) {                          /* ak prvok v tabulke este neexistuje... */
    ...
    ht_item_t *syn = (*table)[index];                  /* skontrolujem, ci na danej pozicii uz nie je nejaky prvok*/
    if (syn != NULL && syn->key != key) {               
      new->next = syn;                                 /* ak ano, a ma rozdielny kluc, nastavim novy prvok, aby bol na zaciatku zoz. synonym */
    }

    (*table)[index] = new;                             /* nakoniec pridam na idealnu poziciu do tabulky novy prvok */

  } else {                     
    tmp->value = value;                           /* v pripade, ze uz dany prvok existuje v tabulke, iba prepisem jeho hodnotu 'value' */
...
void ht_delete_all(ht_table_t *table) {
  ...
  for (int i = 0; i < HT_SIZE; i++) {       /* prechadzam prvky tabulky */
    ...
    if ((*table)[i] != NULL) {                      /* ked najdem prvok, ktory ma nejaku realnu hodnotu, musim uvolnit aj cely jeho zoznam synonym */
      
      tmp2 = (*table)[i];

      while (tmp2->next != NULL) {           /* cyklus while prechadza zoznam syn. a uvolnuje pamet jednotlivych prvkov, okrem toho prveho */
      ...
      free(tmp2);                            /* az nakoniec uvolnim pamet prvku, ktory bol na zaciatku daneho zoznamu syn.*/
