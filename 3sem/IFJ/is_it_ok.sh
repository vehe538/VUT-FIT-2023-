...
#  2024-11-22  Zobecnen vyraz na xCiselny login, varovani pred .git adresarem v archivu
#  2025-11-12  Upraveny identifikatory rozsireni pro IFJ25
#  2025-11-12  Podpora noveho formatu souboru rozdeleni s VUT ID na zacatku kazdeho radku, varovani pri odevzdani nekterych skrytych souboru a adresaru (.DS_Store)
...
function flattening () {
  ...
  for FILE in $FILES; do
    NFILE=./${FILE##*/}  # preparace jmena souboru za poslednim / v ceste (alternativa k basename) a pridani prefixu "./"          
    if [ "$FILE" != "$NFILE" ]; then
  ...
  # Kontrola kolizi souboru (v pripade case-insensitivity na Windows)
...
if [[ -f rozdeleni ]]; then
  ...
  declare -a LOGINS
  {
    ...
    while read -a RADEK; do
      if [[ "${RADEK[0]}" != "" ]]; then  # preskoc prazdny radek (typicky jeden na konci)
        BUTIDS[$i]=${RADEK[0]}
        LOGINS[$i]=${RADEK[1]}
        PERCENTS[$i]=`echo ${RADEK[2]} | tr -cd [:digit:]`
        BADLF=`echo "${RADEK[2]}${RADEK[3]}" | egrep "<LF>|LF"`  # resi se i oddeleni mezerou, coz vytvori bunku s indexem 3
        BADPERCENT=`echo "${RADEK[2]}${RADEK[3]}" | egrep "%"`
        if [[ -n $BADLF ]]; then
          echo_color red "ERROR (bad format: entry $i should be finished by Unix line feed, not string \"<LF>\")"
          let ERROR=ERROR+1
        fi
        if [[ -n $BADPERCENT ]]; then
          echo_color red "ERROR (bad format: percent number in entry $i should not be finished by % symbol, omit it)"
          let ERROR=ERROR+1
        fi
        ((TMP_PROC+=${PERCENTS[$i]:-0}))
        ((i++))
        if [[ "$NAME" = "${RADEK[1]}" ]]; then
          ARCHNAME=$NAME
        fi
        BUTIDCHECK=`echo ${RADEK[0]} | egrep "^([0-9]{5,7})$"`
        if [[ -z $BUTIDCHECK ]]; then
          echo_color red "ERROR (bad BUT id format of ${RADEK[0]})"
          let ERROR=ERROR+1
        fi        
        LOGINCHECK=`echo ${RADEK[1]} | egrep "x([a-z]{5,6}[a-z0-9][a-z0-9]|[0-9]{5,7})$"`
        if [[ -z $LOGINCHECK ]]; then
          echo_color red "ERROR (bad login format of ${RADEK[1]})"
          let ERROR=ERROR+1
        fi
      else
...
if [[ -f rozsireni ]]; then
  ...
  RETCODE=$?
  if [[ $RETCODE = "0" ]]; then
    UNKNOWN=`cat rozsireni | grep -v -E -e "^(FUNEXP|EXTSTAT|EXTFUN|BOOLTHEN|CYCLES|OPERATORS|ONELINEBLOCK|STATICAN)$" | wc -l`
    if [[ $UNKNOWN = "0" ]]; then
      echo_color green "OK" 
    else
...
#   Kontrola adresare .git (skryty)
if [[ -d .git ]]; then
  echo_color blue "Archive ($ARCHIVE) should not contain .git directory (hidden)!"
  let ERROR=ERROR+1
fi
#   Kontrola soubor .DS_Store (skryty)
if [[ -f .DS_Store ]]; then
  echo_color blue "Archive ($ARCHIVE) should not contain .DS_Store file (hidden)!"
  let ERROR=ERROR+1
fi
