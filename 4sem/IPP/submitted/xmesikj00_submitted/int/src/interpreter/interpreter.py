...
from interpreter.class_main import ClassMain
    ...
    def execute(self, input_io: TextIO) -> None:
        ...
        for class_root in root:
            ...
            if class_root.get("name") == "Main":
                # program_frame.print()
                main = ClassMain(class_root, main_frame_stack, class_frame, program_frame)
                has_main = True

            # write class records into program frame
            new_class_obj = ClassRecord(class_root, main_frame_stack, class_frame, program_frame)
            if class_name in program_frame.obj_dict:
                raise InterpreterError(ErrorCode.SEM_ERROR, "Redefinition of class.")
            program_frame.write(class_name, new_class_obj)
