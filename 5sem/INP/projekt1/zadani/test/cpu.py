...
async def test_do_loop(dut):
    """Simple do until loop test"""
    instcnt, mem, lcd = await run_program(dut, '(.-)@\3', timeout_ns = LCD_WAIT_TIME*10)
    assert mem[5] == 0
    assert lcd == "\3\2\1"

@tb_test()
async def test_fastinit(dut):
    """Simple fast initializer test"""
    instcnt, mem, lcd = await run_program(dut, 'A@')
    assert mem[2] == 0xA0
...
async def test_login(dut, uid=''):
    ...
    lcd = lcd.lower()
    assert lcd == "xmesikj00", "Invalid output"
